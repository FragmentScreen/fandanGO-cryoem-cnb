...
def provide_raw_data(project_name, raw_data_links):
    print(f'The link to download the raw_data will be asociated to the project {project_name} ...')
    success = False
    if raw_data_links:
        success = True
        column_names, project_info = get_project_info(project_name)
        exists = any(key == "raw_data_ticket_windows" for _, key, _ in project_info)
